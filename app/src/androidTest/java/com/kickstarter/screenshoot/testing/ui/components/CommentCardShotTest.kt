...
class CommentCardShotTest : ScreenshotTest {
    ...
    fun commentCardScreenshotTest_FLAGGED_MESSAGE() {
        ...
        commentCard.setFlaggedMessage(
            getInstrumentation().targetContext.getString(R.string.This_comment_is_under_review_for_potentially_violating) +
                " " +
                getInstrumentation().targetContext.getString(R.string.Kickstarters_community_guidelines)
        )
        compareScreenshot(commentCard)
    }
}
